...
public class CmdModerator {
	public void Mod (CommandSender sender, Command cmd, String label, String[] args) {
		if(!(sender instanceof Player)) {
			sender.sendMessage("§cMusisz byc na serwerze!");
			return;
		}
		if (args.length != 2) {
			sender.sendMessage("§cPoprawne uzycie: /g mod <gracz>");
			return;
		}
		...
		if (g.getMods().contains(u)) {
			sender.sendMessage("§cTen gracz jest juz moderatorem!");
			return;
		}
		g.addMod(u);
		for (User m : g.getMembers()) {
			m.getPlayer().sendMessage("§aGracz §6§l" + u.getName() + " §azostaje mianowany moderatorem!");
		}
	}
}
